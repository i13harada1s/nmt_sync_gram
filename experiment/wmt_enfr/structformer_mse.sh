...
DATA_DIR=databin/wmt14_enfr
MODEL_DIR=checkpoints/wmt14_enfr_bpe32k/structformer_mse_005
RESULT_DIR=experiments/wmt_enfr/results/bpe32k/structformer_mse
mkdir -p $RESULT_DIR
...
fairseq-generate $DATA_DIR --fp16 \
    --user-dir src \
    --path $MODEL_DIR/checkpoint_best.pt \
    --batch-size 512 --beam 4 --lenpen 0.6 --remove-bpe \
    --results-path $RESULT_DIR

bash $FAIRSEQ_DIR/scripts/sacrebleu.sh wmt14/full en fr $RESULT_DIR/generate-test.txt
...
#     > $RESULT_DIR/structformer_mse.test.en2de.sys.sorted.detok

# poetry run sacrebleu ~/data/wmt16_en_de_bpe32k/newstest2014.de -i $RESULT_DIR/structformer_mse.test.en2de.sys.sorted.detok --language-pair "de-en" -w 2 
