...
MODEL_DIR=checkpoints/align_enfr_spm32k/structformer_rank_${SRC}_${TGT}
mkdir -p $RESULT_DIR
...
MODEL_DIR=checkpoints/align_enfr_spm32k/structformer_rank_${TGT}_${SRC}
mkdir -p $RESULT_DIR
...
echo "Combining bidirectional token-level alignments: ($RESULT_DIR/structformer_rank.${SRC}2${TGT}.talp, $RESULT_DIR/structformer_rank.${TGT}2${SRC}.talp) -> $RESULT_DIR/structformer_rank.enfr.talp"
poetry run python $COMBINE $RESULT_DIR/structformer_rank.${SRC}2${TGT}.talp $RESULT_DIR/structformer_rank.${TGT}2${SRC}.talp --method "grow-diagonal" > $RESULT_DIR/structformer_rank.enfr.talp
