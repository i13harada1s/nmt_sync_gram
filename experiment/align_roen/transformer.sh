...
RESULT_DIR=experiments/align_roen/results/spm32k/transformer

# Garmman -> English

MODEL_DIR=checkpoints/align_roen_spm32k/transformer_${SRC}_${TGT}
mkdir -p $RESULT_DIR
...
MODEL_DIR=checkpoints/align_roen_spm32k/transformer_${TGT}_${SRC}
mkdir -p $RESULT_DIR
...
poetry run python $COMBINE $RESULT_DIR/transformer.${SRC}2${TGT}.talp $RESULT_DIR/transformer.${TGT}2${SRC}.talp --method "grow-diagonal" > $RESULT_DIR/transformer.${SRC}${TGT}.talp

echo "Evaluate $RESULT_DIR/transformer.${SRC}2${TGT}.talp"
poetry run python $EVALUATE $GOLD_ALIGN $RESULT_DIR/transformer.${SRC}2${TGT}.talp --fAlpha 0.5 --oneRef 

echo "Evaluate $RESULT_DIR/transformer.${TGT}2${SRC}.talp"
poetry run python $EVALUATE $GOLD_ALIGN $RESULT_DIR/transformer.${TGT}2${SRC}.talp --fAlpha 0.5 --oneRef --reverseHyp

echo "Evaluate $RESULT_DIR/transformer.${SRC}${TGT}.talp"
poetry run python $EVALUATE $GOLD_ALIGN $RESULT_DIR/transformer.${SRC}${TGT}.talp --fAlpha 0.5 --oneRef 
