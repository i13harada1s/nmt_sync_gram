...
MODEL_DIR=checkpoints/align_jaen_spm32k/structformer_${SRC}_${TGT}
mkdir -p $RESULT_DIR
...
MODEL_DIR=checkpoints/align_jaen_spm32k/structformer_${TGT}_${SRC}
mkdir -p $RESULT_DIR
...
echo "Combining bidirectional token-level alignments: ($RESULT_DIR/structformer.${SRC}2${TGT}.talp, $RESULT_DIR/structformer.${TGT}2${SRC}.talp) -> $RESULT_DIR/structformer.jaen.talp"
poetry run python $COMBINE $RESULT_DIR/structformer.${SRC}2${TGT}.talp $RESULT_DIR/structformer.${TGT}2${SRC}.talp --method "grow-diagonal" > $RESULT_DIR/structformer.jaen.talp
