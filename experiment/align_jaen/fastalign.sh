...
# REFERENCE=$HOME/data/kftt-alignments/data/align.txt
...
EXPERIMENT=experiments/results/kftt_jaen_spm32k/fastalign_tkn
mkdir -p $EXPERIMENT
...
for l in $src $tgt; do
    ...
    cat $DATADIR/train.$l $DATADIR/dev.$l $DATADIR/test.$l > $EXPERIMENT/train.align.plustest.$l
done
info "done."
...
#NOTE(shintaro-h): kftt alignmnet indices start at index 0, --oneRef work bad parformance.
info "${src}-${tgt}"
$ALIGN_SCRIPTS/scripts/aer.py $REFERENCE $EXPERIMENT/${src}_${tgt}.test.talp --fAlpha 0.5
info "${tgt}-${tgt}"
$ALIGN_SCRIPTS/scripts/aer.py $REFERENCE $EXPERIMENT/${tgt}_${src}.test.talp --fAlpha 0.5 --reverseRef
info "${src}-${tgt} bidirectional"
$ALIGN_SCRIPTS/scripts/aer.py $REFERENCE $EXPERIMENT/${src}_${tgt}.test.bidir.${method}.talp --fAlpha 0.5
info "done."
...
DATADIR=data/kftt_jaen_spm32k
EXPERIMENT=experiments/results/kftt_jaen_spm32k/fastalign_spm
mkdir -p $EXPERIMENT
info "create train data for run_fastalign.sh."
for l in $src $tgt; do
    check_file $DATADIR/train.$l
