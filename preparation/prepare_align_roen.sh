...
src=ro
tgt=en
prep=$(pwd)/data/europarl_roen_spm32k
tmp=$prep/tmp
orig=$prep/orig
...
for name in train test; do
    ln -s $DATA_DIR/${name}/roen.lc.src $orig/$name.$src
    ln -s $DATA_DIR/${name}/roen.lc.tgt $orig/$name.$tgt
done
ln -s $DATA_DIR/test/roen.talp $orig/roen.talp
log "Done."
...
for name in train valid; do
    perl -C $CLEAN $tmp/$name $src $tgt $tmp/$name.clean 1 150
    # perl -C $CLEAN -ratio 1.5 $tmp/$name $src $tgt $tmp/$name.clean 1 100
done
...
echo "" > $tmp/train.roen
for lang in $src $tgt; do
    cat $tmp/train.$lang >> $tmp/train.roen
done

log "Building BPE model on ${tmp}/train.roen"
$python $SPM_TRAIN \
    --input=$tmp/train.roen \
