...
src=en
tgt=fr
prep=$(pwd)/data/europarl_enfr_spm32k
tmp=$prep/tmp
orig=$prep/orig
...
for name in train test; do
    ln -s $DATA_DIR/${name}/enfr.lc.src $orig/$name.$src
    ln -s $DATA_DIR/${name}/enfr.lc.tgt $orig/$name.$tgt
done
ln -s $DATA_DIR/test/enfr.talp $orig/enfr.talp
log "Done."
